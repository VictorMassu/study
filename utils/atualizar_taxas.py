...
import os
import json
from tax_binance import obter_taxas_binance
from tax_bybit import obter_taxas_bybit
...
def obter_taxas(par, origem, destino):
    ...
    # Usar taxas específicas para a Binance e Bybit
    if origem == "binance":
        taxas = obter_taxas_binance()
    elif origem == "bybit":
        taxas = obter_taxas_bybit()
    else:
        # Se for uma exchange desconhecida, usamos as taxas padrões
        taxas = {
            "trading": 0.1,
            "saque": {"USDT": 0.3},
            "custo_rede": {"default": 0.2}
        }

    return {
        "trading_origem": taxas.get("trading", 0.1),
        "trading_destino": taxas.get("trading", 0.1),
